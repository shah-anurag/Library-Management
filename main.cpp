...
public:
    ...
    Resource(author, title, Id, DateOfPurchase, Edition, RackNumber), SubjectCode(SubCode), name(title){}
    void Update();
    void AddSubject();
...
public:
    ...
    {
        for(unsigned i = 0U; i < history.size(); i++)
        {
...
void Library::AddMember()
{
    if(first)
    {
        ...
        long long ph_num;
        Staff* staff = new Staff(name = "Anurag", JoinDate = "02/04/2018", Id = "Sadmin", ph_num = 8320130726LL, password = "admin", address = "Office of academics", emailId = "anurag.2@iitj.ac.in");
...
void User::updateHistory(string Resid, Library &lib)
{
    ...
    if(getId()[0] == 'F' && Resid[0] == 'B')
        history.push_back(new BookIssued(res, Faculty::getBookLimit()));
    else if(getId()[0]=='F' && Resid[0] == 'J')
        history.push_back(new BookIssued(res, Faculty::getJournalLimit()));
    else if(getId()[0]=='P' && Resid[0] == 'B')
        history.push_back(new BookIssued(res, Phd::getBookLimit()));
    else if(getId()[0]=='P' && Resid[0] == 'J')
        history.push_back(new BookIssued(res, Phd::getJournalLimit()));
    else if(getId()[0]=='N' && Resid[0] == 'B')
        history.push_back(new BookIssued(res, NonPhd::getBookLimit()));
    return;
}
